...
let matches = JSON.parse(localStorage.getItem('tournament-matches')) || [];
...
function generateTournamentMatches() {
    ...
    games.forEach(game => {
        ...
        const totalPairs = Math.min(matchesPerGame, Math.floor(teams.length / 2));
        
        console.log(`  📈 Generando ${totalPairs} partidas para ${game.name}`);
        ...
        for (let i = 0; i < totalPairs; i++) {
            ...
            if (team1Index < shuffledTeams.length && team2Index < shuffledTeams.length) {
                const match = {
                    id: matchId++,
                    round: roundNumber,
                    team1: shuffledTeams[team1Index],
                    team2: shuffledTeams[team2Index],
                    game: game,
                    completed: false,
                    winner: null,
                    timestamp: new Date().toISOString()
    ...
    console.log(`✅ ${matches.length} partidas generadas en ${games.length} rondas`);
...
function declareWinner(matchId, winnerId) {
    ...
    alert(`🎉 ¡${winner.name} gana la partida de ${match.game.name}!`);
    
    console.log(`🏆 ${winner.name} venció a ${loser.name} en ${match.game.name}`);
}
...
function showMatchDistribution() {
    ...
    Object.keys(matchesByRound).forEach(round => {
        ...
        const gameName = roundMatches[0]?.game.name || 'Desconocido';
